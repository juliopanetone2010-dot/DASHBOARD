...
new_content = []
# We keep lines before 167 (1-indexed)
new_content.extend(lines[:166])
...
# Find where the old grid starts (should be around line 238 in original or later in messed up version)
# We want to keep everything from "<div className=\"grid grid-cols-1 md:grid-cols-2 gap-4\">" onwards.
start_keeping = -1
...
if start_keeping != -1:
    new_content.extend(lines[start_keeping:])
else:
