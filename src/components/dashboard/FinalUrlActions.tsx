...
export function FinalUrlActions({ url, source, trackingTemplate, finalUrlSuffix, mobileUrl, className, compact }: Props) {
      ...
      <TooltipProvider delayDuration={150}>
        <Tooltip>
          <TooltipTrigger asChild>
            <span className={cn("inline-flex items-center gap-1 text-[11px] text-destructive/80", className)}>
          ...
          <TooltipContent side="bottom" className="max-w-[420px] text-xs">
            ...
            {!trackingTemplate && !finalUrlSuffix && <div className="text-muted-foreground">Rode "Sincronizar URLs" para puxar da API.</div>}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
