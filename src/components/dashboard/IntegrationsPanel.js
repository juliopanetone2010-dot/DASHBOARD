...
export function IntegrationsPanel(props) {
    ...
    useEffect(() => {
        (async () => {
            const { data } = await supabase.functions.invoke("google-ads-oauth-status");
            if (data) {
                setOauthStatus(data);
                if (data.default_api_set)
                    setApiSet(data.default_api_set);
            }
        })();
    }, []);
    ...
    const handleSyncGam = async () => {
        ...
        const totalRows = summary.reduce((acc, s) => acc + (Number(s.ad_unit_rows) || 0) + (Number(s.placement_rows) || 0), 0);
    ...
    const handleSyncCampaigns = async (deep = false) => {
        ...
        const total = (data?.summary ?? []).reduce((acc, s) => {
            const x = s;
    ...
    const handleSaveDevToken = async (e) => {
        e.preventDefault();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border bg-card p-5 shadow-elegant">
          ...
          <div className="space-y-1 mb-3">
            ...
            <select className="w-full h-9 rounded-md border border-input bg-background px-3 text-sm" value={apiSet} onChange={(e) => setApiSet(Number(e.target.value))}>
              {[1, 2, 3, 4, 5].map((i) => {
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {apiSets.map((s) => (<span key={s.api_set} className={`inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[10px] ${s.configured ? "border-success/40 text-success" : "border-border text-muted-foreground"}`}>
            ...
            <form onSubmit={handleSaveDevToken} className="mt-2 space-y-2 border-t border-border/50 pt-2">
              <div className="flex flex-col gap-1">
                ...
                <div className="flex flex-col gap-2">
                  <Input type="password" value={manualDevToken} onChange={(e) => setManualDevToken(e.target.value)} placeholder="Insira o Developer Token aqui..." className="h-8 text-xs"/>
                  <div className="flex gap-2">
                    <Input type="password" value={manualClientId} onChange={(e) => setManualClientId(e.target.value)} placeholder="Client ID (Opcional se global)" className="h-8 text-xs flex-1"/>
                    <Input type="password" value={manualClientSecret} onChange={(e) => setManualClientSecret(e.target.value)} placeholder="Client Secret (Opcional)" className="h-8 text-xs flex-1"/>
                  </div>
                  <Button onClick={handleSaveDevToken} size="sm" className="h-8 w-full" disabled={savingSecret}>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <Button onClick={handleConnectAds} size="sm" className="gap-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700" disabled={connecting}>
              {connecting ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Plug className="h-3.5 w-3.5"/>}
              1. Conectar MCC
            </Button>

            <Button onClick={handleListAccounts} size="sm" variant="secondary" className="gap-1.5" disabled={listingAccounts || connecting}>
          ...
          {showAccountSelector && (<div className="mt-4 p-4 border border-dashed border-border rounded-lg bg-muted/20 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {accessibleAccounts.length === 0 ? (<div className="text-center py-4 space-y-2">
                  ...
                  </div>) : (accessibleAccounts.map((mcc) => (<div key={mcc.manager} className="space-y-1.5 mb-3 last:mb-0">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">MCC: {mcc.manager}</p>
                      {mcc.error ? (<p className="text-[10px] text-destructive italic">{mcc.error}</p>) : mcc.synced > 0 ? (<div className="flex items-center gap-2 text-xs text-success bg-success/5 p-2 rounded border border-success/20">
                          <CheckCircle2 className="h-3.5 w-3.5"/>
                          {mcc.synced} conta(s) sincronizada(s) e disponível(is) para mapeamento.
                        </div>) : (<p className="text-xs text-muted-foreground italic">Nenhuma conta ativa encontrada nessa MCC.</p>)}
                    </div>)))}
              </div>
              {accessibleAccounts.length > 0 && (<p className="text-[10px] text-muted-foreground mt-3 italic">
        ...
        <div className="space-y-4">
          <form onSubmit={handleAddGam} className="rounded-xl border border-border bg-card p-5 shadow-elegant space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <Button type="button" size="sm" variant="secondary" disabled={syncingGam} onClick={handleSyncGam} className="h-7 text-[11px] gap-1">
          ...
          <form onSubmit={handleAddManualAccount} className="rounded-xl border border-border bg-card p-5 shadow-elegant space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <Input value={manualAccountName} onChange={(e) => setManualAccountName(e.target.value)} placeholder="Minha Conta" className="h-8 text-xs"/>
              </div>
              <div className="space-y-1">
                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Customer ID</Label>
                <Input value={manualCustomerId} onChange={(e) => setManualCustomerId(e.target.value)} placeholder="123-456-7890" className="h-8 text-xs"/>
              </div>
            </div>
