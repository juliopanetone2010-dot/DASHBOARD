...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const listRes = await fetch(`https://googleads.googleapis.com/v25/customers/${mccAccount.customer_id}/googleAds:search`, {
      method: "POST",
      headers: {
    ...
    for (const child of children) {
      ...
      const camRes = await fetch(`https://googleads.googleapis.com/v25/customers/${childCid}/googleAds:search`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "developer-token": devToken,
