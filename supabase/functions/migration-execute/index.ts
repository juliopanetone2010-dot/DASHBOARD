...
async function runMigration(a: RunArgs) {
  ...
  const srcBase = `https://googleads.googleapis.com/v17/customers/${srcAcc.customer_id}`;
  const dstBase = `https://googleads.googleapis.com/v17/customers/${dstAcc.customer_id}`;
  ...
  for (const ref of refs) {
    ...
    try {
      if (asset.type === "IMAGE" && asset.imageAsset?.fullSize?.url) {
        ...
        // Truque do Google Ads Editor: o recurso legado `media_file` (mesmo ID que o asset)
        // expõe `media_file.media_bundle.url` — URL pública do ZIP no tpc.googlesyndication.com.
        // Baixamos por HTTP e re-uploadamos como MEDIA_BUNDLE asset na conta destino.
